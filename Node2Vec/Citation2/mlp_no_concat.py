...
class LinkPredictor(torch.nn.Module):
    ...
    def forward(self, x_i, x_j):
        # print(x_i.shape) # (65536, 128)
        # print("---")
        x = x_i * x_j
        # print(x.shape) # (65536, 128)
...
def main():
    ...
    parser.add_argument('--dst_dir', default="models_no_concat")
    ...
    embedding_path = os.path.join(embedding_dir, str(args.dim), str(args.seed_id)+".pt")
    ...
    os.makedirs(os.path.join(args.dst_dir, str(args.dim)), exist_ok=True)
    model_save_path = os.path.join(args.dst_dir, str(args.dim), str(args.seed_id)+".pth")
