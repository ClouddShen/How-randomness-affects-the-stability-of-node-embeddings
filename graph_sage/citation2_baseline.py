# reference: https://github.com/snap-stanford/ogb/blob/master/examples/linkproppred/citation2/gnn.py
...
def save_embedding_data(node_embbeding, embed_dim, seed):
    file_path = f'./citation2/embedding_data/{str(embed_dim)}/{str(seed)}'
...
@torch.no_grad()
def test(model, predictor, data, split_edge, evaluator, batch_size, embed_dim, seed):
    predictor.eval()

    h = model(data.x, data.adj_t)
    print("Saving node embeddings...")
    save_embedding_data(h, embed_dim, seed)
...
def main(seed, embed_dim):
    ...
    predictor = LinkPredictor(embed_dim, embed_dim, 1,
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            if epoch % args.eval_steps == 0:
                result = test(model, predictor, data, split_edge, evaluator,
                              args.batch_size, embed_dim, seed)
                logger.add_result(run, result)

                if epoch % args.log_steps == 0:
                    print("Saving GraphSAGE...")
                    model_path = f'./citation2/model/{embed_dim}/GraphSAGE_embed_{embed_dim}_seed_{seed}.pt'
                    torch.save(model.state_dict(), model_path)
                    print("Saving MLP...")
                    predictor_path = f'./citation2/model/{embed_dim}/MLP_embed_{embed_dim}_seed_{seed}.pt'
